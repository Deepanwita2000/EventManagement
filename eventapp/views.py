...
def add_events(request):
    if request.method == "POST":
        ...
        banner =request.FILES.get("banner_image")   # request.FILES.get("stream_image")
        status=request.POST.get("status")
        print(event_title,description,location,date,time)
        #   createdBy
        if event_title:
            #create event on db
...
#     stream: Stream = get_object_or_404(Stream, pk=pk) if pk else None

#     if request.method == "POST":
#         name: str = request.POST.get("stream_name", "").strip()
#         stream_description: str = request.POST.get("stream_description", "").strip()
#         new_image = request.FILES.get("stream_image")  # uploaded file

#         if name and stream:
#             # Exclude current stream from duplicate check
#             if not Stream.objects.filter(name=name).exclude(pk=stream.pk).exists():
#                 stream.name = name
#                 stream.description = stream_description
#                 if new_image:
#                     # Delete old image if exists
#                     if stream.image and os.path.isfile(stream.image.path):
#                         os.remove(stream.image.path)
...
#                 html_string = render_to_string("partials/stream_rows.html", {"streams": streams})
#                 return JsonResponse({"streams": html_string, "message": "Stream updated successfully!"})
#             else:
