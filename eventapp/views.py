...
from accountapp.permission import IsOrganizerOrUser,IsOrganizer,IsUser
...
class EventViewSet(ModelViewSet):
    ...
    #serach by location
     ...
     #search by category name -filter
    # @action(detail=True , methods=['get'],url_path='filter-events' , permission_classes=[AllowAny] , authentication_classes=[])
    # def filter_events(self, request , pk=None):
    #     filter_category = Event.objects.filter(category=pk)
    #     serializer = self.get_serializer(filter_category,many=True)
    #     print(serializer)
    #     return Response(serializer.data)
