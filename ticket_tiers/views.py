...
class ReadAllTickets(ModelViewSet):
...
class TierViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        print(event)
        # if Tier.objects.filter(event=event.title , organizer=user).exists():
        #     raise exceptions.ValidationError("organizer with this event is already registered withthis ticket.")

        if user.id != requested_id:
            raise PermissionDenied("You are not allowed to create other's ticket")

        if Tier.objects.filter(event=event.id , name=name).exists():
            raise exceptions.ValidationError("event is already registered with this ticket.")
        serializer.save(organizer=user)
  
    # view tickets created by the specific authenticated organizer
    ...
    @action(detail=False , methods=['get'],url_path='all-tickets' , permission_classes=[AllowAny] , authentication_classes=[])
    def all_tickets(self, request):
        all_tickets = Tier.objects.all()
        serializer = self.get_serializer(all_tickets,many=True)
        print(serializer)
        return Response(serializer.data)
